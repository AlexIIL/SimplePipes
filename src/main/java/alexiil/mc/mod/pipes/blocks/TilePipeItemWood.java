...
public class TilePipeItemWood extends TilePipeWood {
    ...
    public TilePipeItemWood(BlockPos pos, BlockState state) {
        super(
            SimplePipeBlocks.WOODEN_PIPE_ITEM_TILE, pos, state, SimplePipeBlocks.WOODEN_PIPE_ITEMS, PipeFlowItem::new
        );
    }
    ...
    public void tryExtract(Direction dir, int pulses) {
        ...
        if (!stack.isEmpty()) {
            ((PipeFlowItem) getFlow()).insertItemsForce(stack, dir, null, EXTRACT_SPEED);
        }
    }
}
